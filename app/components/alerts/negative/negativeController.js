...
angular.module('webix')
    .controller('negativeController',function ($scope,$rootScope,$http,auth,$interval,$state,webNotification,searchQueryAlert,$mdToast,$filter, $mdDialog) {
        if(auth.isAuthed()){
            $scope.score = [];
            var getScore = function (value) {
                if (auth.isAuthed()){
                    if ($scope.score.length === 0 || value === 1)
                        $http.get($scope.apiUrl + "/scores/list_score_by_student/?student_id=" + $rootScope.selectedStudent.id).then(function (response) {
